<?php
// 每20分钟执行一次的脚本 */20 6-23 * * * php /var/www/html/20161101/trade22.php
	...
	if($isHoliday == 0){
		$sql = "select * from historytradenum where isHoliday=0 and trade_type = 22 and time = '$time7'";
	}else{
		$sql = "select * from historytradenum where isHoliday=1 and trade_type = 22 and time = '$time7'";
	}
	...
	//实时接入量（线网）
	$sql = "select count(*) from realt where RECEIVE_DATE > '$time4' and RECEIVE_DATE <= '$time5' and TRADE_DATE > '$time' and TRADE_DATE <= '$time0' and TRADE_TYPE = 22";
	$result = mysql_query($sql);
	...
	//各线路
	$sql = "select station.line_name,count(*) from realt,station where realt.TRADE_ADDRESS = station.station_id and RECEIVE_DATE > '$time4' and RECEIVE_DATE <= '$time5' and TRADE_DATE > '$time' and TRADE_DATE <= '$time0' and TRADE_TYPE = 22 GROUP BY station.line_name";
	$result = mysql_query($sql);
	...
	$new_sql = "insert into trade22 values(null,$start,$all,$tmp,1,'$time7',$error,$isHoliday)";
	mysql_query($new_sql);

	//历史平均（延时大于20）
	if($isHoliday == 0){
		$sql = "select * from historytradescale where isHoliday=0 and trade_type=22 and time='$time7'";
	}else{
		$sql = "select * from historytradescale where isHoliday=1 and trade_type=22 and time='$time7'";
	}
	$result = mysql_query($sql);
	$arr_history_num = mysql_fetch_array($result);
	//新建一个数据用来存放各条线路的偏差
	...
	//平均延时（线网）
	$sql3 = "select TRADE_DATE,RECEIVE_DATE from realt where RECEIVE_DATE > '$time4' and RECEIVE_DATE <= '$time5' and TRADE_DATE > '$time2' and TRADE_TYPE = 22";
	...
	//各线路
	$sql3 = "select station.line_name,TRADE_DATE,RECEIVE_DATE from realt,station where realt.TRADE_ADDRESS = station.station_id and RECEIVE_DATE > '$time4' and RECEIVE_DATE <= '$time5' and TRADE_DATE > '$time2' and TRADE_TYPE = 22";
	$result = mysql_query($sql3);
	...
	$new_sql = "insert into trade22 values(null,$start,$all,$tmp,3,'$time7',null,$isHoliday)";
	mysql_query($new_sql);
	...
	if ($error = 1) {
		...
		$sql_error_num_old = "select error from trade22 where num = 4 and time >= '$time6' ORDER BY time DESC limit 1";
		$result = mysql_query($sql_error_num_old);
		if ($row = mysql_fetch_row($result)) {
			$error = $row[0] + 1;
		}
	}
	$new_sql = "insert into trade22 values(null,$start,$all2,$tmp,4,'$time7',$error,$isHoliday)";
	mysql_query($new_sql);

	//累计数据接入量
	$sql = "select SUM(全网线),SUM(11号线),SUM(1号线),SUM(2号线),SUM(3号线),SUM(4号线),SUM(5号线),SUM(7号线),SUM(9号线) from trade22 where time>='$time6' and time<='$start' and num=1";
	$result = mysql_query($sql);
	$row = mysql_fetch_array($result);
	//插入
	$tmp = $row[0].','.$row[1].','.$row[2].','.$row[3].','.$row[4].','.$row[5].','.$row[6].','.$row[7].','.$row[8];
	$new_sql = "insert into trade22 values(null,$start,$tmp,5,'$time7',null,$isHoliday)";
	mysql_query($new_sql);

	// 判断当天的数据是否异常（给代号用）
	$sql_error_num = "select error from trade22 where num =4 and time >= '$time6' order by error desc limit 1";
