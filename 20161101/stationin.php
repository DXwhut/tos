...
echo $time5." begin to insert\n";
	...
	if($isHoliday == 0){
		$sql = "select * from historytradenum where isHoliday=0 and trade_type = 21 and time = '$time7'";
	}else{
	...
	$new_sql = "insert into test21 values(null,$start,$all,$tmp,1,'$time7',$error,$isHoliday)";
	mysql_query($new_sql);

	//历史平均（延时大于20）
	if($isHoliday == 0){
		$sql = "select * from historytradescale where isHoliday=0 and trade_type=21 and time='$time7'";
	}else{
	...
	//平均延时（线网）
	$sql3 = "select TRADE_DATE,RECEIVE_DATE from realt where RECEIVE_DATE > '$time4' and RECEIVE_DATE <= '$time5' and TRADE_TYPE = 21";
	...
	//各线路
	$sql3 = "select station.line_name,TRADE_DATE,RECEIVE_DATE from realt,station where realt.TRADE_ADDRESS = station.station_id and RECEIVE_DATE > '$time4' and RECEIVE_DATE <= '$time5' and TRADE_TYPE = 21";
	$result = mysql_query($sql3);
	...
	$new_sql = "insert into test21 values(null,$start,$all,$tmp,3,'$time7',$error,$isHoliday)";
	mysql_query($new_sql);
	...
	$new_sql = "insert into test21 values(null,$start,$all2,$tmp,4,'$time7',$error,$isHoliday)";
	mysql_query($new_sql);

	//累计数据接入量
	$sql = "select SUM(全网线),SUM(11号线),SUM(1号线),SUM(2号线),SUM(3号线),SUM(4号线),SUM(5号线),SUM(7号线),SUM(9号线) from test21 where time>='$time6' and time<='$start' and num=1";
	$result = mysql_query($sql);
	$row = mysql_fetch_array($result);
	//插入
	$tmp = $row[0].','.$row[1].','.$row[2].','.$row[3].','.$row[4].','.$row[5].','.$row[6].','.$row[7].','.$row[8];
	$new_sql = "insert into test21 values(null,$start,$tmp,5,'$time7',$error,$isHoliday)";
	mysql_query($new_sql);
