...
header("Access-Control-Allow-Origin: *");//运行跨域
	...
	if($trade_type == 'singleTicket'){
		$sql = "select insert_time,$line from trade01 where time > '$time' and time < '$time2' and num = 1";
		$sql2 = "select time,$line from historytradenum where trade_type = 1 and isHoliday=$isHoliday";
		$result = mysql_query($sql);
		...
		$sql = "select insert_time,$line from trade21 where time > '$time' and time < '$time2' and num = 1";
		$sql2 = "select time,$line from historytradenum where trade_type = 21 and isHoliday=$isHoliday";
		$result = mysql_query($sql);
		$result2 = mysql_query($sql2);
		while ($row = mysql_fetch_row($result)) {
			$arr[$row[0]]['time'] = $row[0];
		...
		$sql = "select insert_time,$line from trade22 where time > '$time' and time < '$time2' and num = 1";
		$sql2 = "select time,$line from historytradenum where trade_type = 22 and isHoliday=$isHoliday";
		$result = mysql_query($sql);
		$result2 = mysql_query($sql2);
		while ($row = mysql_fetch_row($result)) {
			$arr[$row[0]]['time'] = $row[0];
